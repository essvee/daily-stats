...
def update_publishers():
    ...
    urls = ["http://api.gbif.org/v1/organization/deleted", "http://api.gbif.org/v1/organization"]

    for url in urls:
        try:
            count = requests.get(f"{url}?limit=0").json()['count']
            offset = 0
            while count > offset:
                results = requests.get(f"{url}?limit=200&offset={offset}").json()['results']
                for p in results:
                    # If org is new, add the name, key and country to the table
                    if p['key'] not in pub_keys:
                        publisher_name = p['title'].replace("\"", "\'").encode('ascii', 'ignore').decode('utf-8',
                                                                                                         'ignore')
